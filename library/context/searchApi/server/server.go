...
import (
	"context"
	"go-example/context/searchApi/google"
...
func handleSearch(w http.ResponseWriter, req *http.Request) {
	...
	start := time.Now()
	reqVal := func(in, def string) string {
		if in != "" {
			return in
		}
		return def
	}
	querys := google.Querys{
		"q":     query,
		"num":   reqVal(req.FormValue("num"), "10"),
		"start": reqVal(req.FormValue("start"), "1"),
	}
	results, err := google.Search(ctx, querys)
